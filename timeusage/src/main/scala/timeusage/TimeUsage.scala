...
object TimeUsage extends TimeUsageInterface {
  ...
  def timeUsageByLifePeriod(): Unit = {
    val (columns, initDf) = read("src/main/scala/timeusage/atussum.csv")
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val pPrefix = List("t01","t03","t11","t1801","t1803")
    val wPrefix = List("t05","t1805")
    val oPrefix = List("t02","t04","t06","t07","t08","t09","t10","t12","t13","t14","t15","t16","t18")
    def isPresent(name:String , prefixes: List[String]):Boolean = {
      prefixes.exists( prefix => name.startsWith(prefix))
    }
    val primary = columnNames.filter(co => isPresent(co,pPrefix)).map(col)
    val working = columnNames.filter(co => isPresent(co,wPrefix)).map(col)
    val other = columnNames.filter(co => isPresent(co,oPrefix))
      .filter(p => !isPresent(p,pPrefix))
      .filter(w => !isPresent(w,wPrefix)).map(col)
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when(df("teage")>=15 && df("teage")<23,"young")
      .when(df("teage")>=23 &&df("teage")<56, "active").otherwise("elder").as("age")

    // Create columns that sum columns of the initial dataset
    // Hint: you want to create a complex column expression that sums other columns
    //       by using the `+` operator between them
    // Hint: don’t forget to convert the value to hours
    def sumCol(ls:List[Column]):Column = ls.reduceLeft(_+_)/lit(60)
    val primaryNeedsProjection: Column = sumCol(primaryNeedsColumns).as("primaryNeeds")
    val workProjection: Column = sumCol(workColumns).as("work")
    val otherProjection: Column = sumCol(otherColumns).as("other")
    df
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(x => TimeUsageRow(x.getAs("working"),x.getAs("sex"),x.getAs("age"),x.getAs("primaryNeeds")
    ,x.getAs("work"),x.getAs("other")))
